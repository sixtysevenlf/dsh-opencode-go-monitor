...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, analyzeMode: 'off', recCostLimit: 0, recOffpeakHourly: 0, recPeakHourly: 0, recPeakPerProject: 0, recParallel: 1, recDaily: 0, gateOn: false, gatePeakOnly: false, compressOn: false, compressAtPercent: 80, compressPeakOnly: true, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
        ...
        (props) => {
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              ...
              }, '近' + (h.avg.activeDays || 7) + '天日均: ' + fmtTokens(h.avg.tokens) + ' tok · $' + h.avg.cost.toFixed(3) + '/日' + autoInfo));
            }
            ...
            if (settings.analyzeMode === 'lowcost' && Number(settings.recDaily) > 0) {
              ...
              dsChildren.push(React.createElement('div', {
                ...
                React.createElement('button', {
                  ...
                  title: '把分析建议套用到手动限额：每日 tok=' + fmtTokens(settings.recDaily) + '、每小时=' + fmtTokens(settings.recOffpeakHourly) + '（高峰自动减半=建议高价区值）、每日金额=$' + Number(settings.recCostLimit).toFixed(3),
