...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, reqLimit: 0, costLimit: 0, peakReqLimit: 0, autoLimit: false };
    ...
    function BalanceSettingsRow() {
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r3', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', { key: 'rl', style: { color: 'var(--dsw-alias-label-secondary,#888)' } }, '每日请求上限'),
          React.createElement('input', {
            key: 'req',
            type: 'number',
            min: 0,
            style: inputStyle,
            value: settings.reqLimit || '',
            placeholder: '不限',
            onChange: (e) => update({ reqLimit: num(e.target.value) }),
          }),
          React.createElement('span', { key: 'cl', style: { color: 'var(--dsw-alias-label-secondary,#888)' } }, '每日费用上限 $'),
          React.createElement('input', {
            key: 'cost',
            type: 'number',
            min: 0,
        ...
        React.createElement('div', { key: 'r4', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', { key: 'pl', style: { color: 'var(--dsw-alias-label-secondary,#888)' } },
            '高峰期请求上限（09-12 / 14-18 内）'),
          React.createElement('input', {
            key: 'peak',
            type: 'number',
            min: 0,
            style: inputStyle,
            value: settings.peakReqLimit || '',
            placeholder: '不限',
            onChange: (e) => update({ peakReqLimit: num(e.target.value) }),
          }),
        ),
        React.createElement('div', { key: 'r5', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          ...
          React.createElement('input', {
            key: 'alc',
            type: 'checkbox',
            checked: settings.autoLimit === true,
            onChange: (e) => update({ autoLimit: e.target.checked }),
          }),
        ),
      );
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 自动限额：开启时按近 7 天日均用量校准每日/高峰/费用上限（用量每次刷新时校准）
          React.useEffect(() => {
            if (settings.autoLimit !== true) return;
            const h = use && use.history;
            if (!h || !h.avg) return;
            const nextReq = h.avg.msgs;
            const nextCost = Math.round(h.avg.cost * 100) / 100;
            const nextPeak = h.avg.peakMsgs;
            if (settings.reqLimit !== nextReq || settings.costLimit !== nextCost || settings.peakReqLimit !== nextPeak) {
              saveSettings({ ...settings, reqLimit: nextReq, costLimit: nextCost, peakReqLimit: nextPeak });
            }
          }, [use, settings]);
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              ...
              }, '近7天日均: ' + use.history.avg.msgs + ' 次 · $' + use.history.avg.cost.toFixed(3) + '/日'
                + (settings.autoLimit === true ? '（自动限额: 请求' + settings.reqLimit + ' · 高峰' + settings.peakReqLimit + ' · $' + settings.costLimit + '）' : '')));
            }
          }
          // ---- 省钱：限额监控（省钱模式开启时显示进度条） ----
          const saveOn = settings.saveMode === true;
          if (saveOn && an && an.today) {
            const req = an.today.msgs, cost = an.today.cost;
            const reqCap = Number(settings.reqLimit) || 0;
            const costCap = Number(settings.costLimit) || 0;
            const peakReq = an.peak ? an.peak.msgs : 0;
            const peakReqCap = Number(settings.peakReqLimit) || 0;
            const peakOver = peakReqCap > 0 && peakReq > peakReqCap;
            const over = (reqCap > 0 && req > reqCap) || (costCap > 0 && cost > costCap) || peakOver;
            ...
            dsChildren.push(budgetRow('请求', req + (reqCap > 0 ? '/' + reqCap : ''), req, reqCap));
            dsChildren.push(budgetRow('费用', '$' + cost.toFixed(4) + (costCap > 0 ? '/$' + costCap.toFixed(2) : ''), cost, costCap));
            dsChildren.push(budgetRow('高峰', peakReq + (peakReqCap > 0 ? '/' + peakReqCap : ''), peakReq, peakReqCap));
            if (over) {
              dsChildren.push(React.createElement('div', {
                key: 'over',
                style: { fontSize: 11, fontWeight: 600, color: '#DC2626' },
              }, peakOver ? '⚠ 高峰时段已达请求上限，建议暂停或改空闲时段使用' : '⚠ 已达上限，建议暂停使用'));
            }
          }
          dsChildren.push(React.createElement('div', { key: 'save-btns', style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 } },
            ...
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
              title: '自动限额：按近7天日均用量设置每日/高峰/费用上限',
              onPointerDown: (e) => e.stopPropagation(),
              onClick: (e) => {
                e.stopPropagation();
                const s = loadSettings();
                saveSettings({ ...s, autoLimit: !s.autoLimit });
              },
            }, settings.autoLimit === true ? '自动:开' : '自动:关'),
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
              title: '把上限设为今日已用量（超限提醒）',
              onPointerDown: (e) => e.stopPropagation(),
              onClick: (e) => {
                e.stopPropagation();
                const s = loadSettings();
                if (an && an.today) {
                  saveSettings({
                    ...s,
                    saveMode: true,
                    reqLimit: an.today.msgs,
                    costLimit: Math.round(an.today.cost * 100) / 100,
                    peakReqLimit: an.peak ? an.peak.msgs : 0,
                  });
                }
              },
            }, '一键限额'),
