...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, autoCostLimit: 0, autoTokenLimit: 0, autoHourlyTokenLimit: 0, autoMode: 'off', gateOn: false, gatePeakOnly: false, compressOn: false, compressAtPercent: 80, compressPeakOnly: true, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
    ...
    // 生效限额：手动与自动分开存储；自动开启且有值时优先自动
    const effCaps = (s) => {
      const autoOn = s.autoMode !== 'off';
      const mCost = Number(s.costLimit) || 0;
      const mTok = Number(s.tokenLimit) || 0;
      const mHour = Number(s.hourlyTokenLimit) || 0;
      const aCost = Number(s.autoCostLimit) || 0;
      const aTok = Number(s.autoTokenLimit) || 0;
      const aHour = Number(s.autoHourlyTokenLimit) || 0;
      return {
        cost: autoOn && aCost > 0 ? aCost : mCost,
        tok: autoOn && aTok > 0 ? aTok : mTok,
        hour: autoOn && aHour > 0 ? aHour : mHour,
        autoOn,
      };
    };
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 自动限额：7天平均 或 单日小时平均（写入独立的 auto* 字段，不覆盖手动限额）
          React.useEffect(() => {
            ...
            if (mode === '7d') {
              if (!h.avg) return;
              const nTok = h.avg.tokens;
              const nCost = Math.round(h.avg.cost * 100) / 100;
              if (settings.autoTokenLimit !== nTok || settings.autoCostLimit !== nCost) {
                saveSettings({ ...settings, autoTokenLimit: nTok, autoCostLimit: nCost });
              }
            } else if (mode === 'hourly') {
              if (!h.refDay) return;
              const nHour = Math.max(1, Math.round(h.refDay.tokens / 24));
              const nTok = h.refDay.tokens;
              const nCost = Math.round(h.refDay.cost * 100) / 100;
              if (settings.autoHourlyTokenLimit !== nHour || settings.autoTokenLimit !== nTok || settings.autoCostLimit !== nCost) {
                saveSettings({ ...settings, autoHourlyTokenLimit: nHour, autoTokenLimit: nTok, autoCostLimit: nCost });
              }
            }
          }, [use, settings]);
          ...
          }, [settings.gateOn, settings.gatePeakOnly, settings.costLimit, settings.tokenLimit, settings.autoMode,
              settings.autoCostLimit, settings.autoTokenLimit, settings.compressOn, settings.compressAtPercent, settings.compressPeakOnly]);
          ...
          React.useEffect(() => {
            ...
            const effHour = (caps.autoOn && hourlyCapRaw > 0 && isPeakNow)
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              ...
              if (settings.autoMode === '7d') {
                autoInfo = '（自动限额: 7天平均 · tok ' + fmtTokens(settings.autoTokenLimit) + ' · $' + Number(settings.autoCostLimit).toFixed(2) + '）';
              } else if (settings.autoMode === 'hourly') {
                const hc = Number(settings.autoHourlyTokenLimit) || 0;
          ...
          if (saveOn && an && an.today) {
            ...
            const hourlyCapRaw = caps.hour;
            // 自动小时模式：高峰时段每小时 tok 上限自动减半（花费等价）
            const effHourlyCap = (caps.autoOn && hourlyCapRaw > 0 && isPeakNowAny)
              ? Math.max(1, Math.round(hourlyCapRaw / 2))
              : hourlyCapRaw;
