            ...
            onChange: (e) => {
              ...
              if (next !== 'off' && settings.analyzeMode === 'off') {
                const ok = window.confirm(
                  '分析模式说明\n\n此模式「分析」你的真实用量并给出建议限额（数据优先来自官网平台当月统计，未配置官网凭据时回退本地分析）：\n\n· 每日建议 = 官网当月日均 token / 金额（活跃日平均）\n· 每小时建议 = 日均 ÷ 24；高价时段 2 倍价 → 建议减半（每小时上限高峰自动减半）\n· 考虑多项目并行：按本地观测的并行项目数给出「每项目每小时」\n· 不会自动修改任何限额；点「采纳」一键套用到手动限额\n· 关闭后建议值保留，随时可采纳\n\n确定开启？',
                );
                if (!ok) return;
              }
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          React.useEffect(() => {
            ...
            if (!puOk && !lc) return;
            // 基准每小时用量：官网可用 → 官网日均 tok ÷ 24（全量口径）；否则回退本地低价区活跃速率
            const recOff = puOk
              ? Math.round(pu.dailyAvgTokens / 24)
              : (lc.offpeakHourly || lc.peakHourly || 0);
            const recPeak = Math.round(recOff / 2);                              // 高价区 2 倍价 → 建议每小时减半
            // 并行项目数：本地 opencode 观测（官网无项目维度），向上取整
            const parallel = Math.max(1, Math.ceil((lc && lc.parallelAvg) || 1));
            const recPer = Math.max(1, Math.round(recPeak / parallel));          // 每项目每小时
            // 每日建议：官网 → 官网日均 tok/金额（活跃日平均）；回退 → 本地速率×活跃日时长 / 本地官网价日均
            const recDaily = puOk ? pu.dailyAvgTokens : (recOff > 0 ? (lc.dailyEstimate || 0) : 0);
            const rCost = Math.round((puOk ? pu.dailyAvgCost : (h.avg ? h.avg.cost : 0)) * 100) / 100;
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              ...
              if (settings.analyzeMode === 'lowcost') {
                autoInfo = '（分析: 每日 ≈' + fmtTokens(settings.recDaily) + ' tok · ' + fmtMoney(Number(settings.recCostLimit), settings.currency, rate) + '/日 · 每小时 ≈' + fmtTokens(settings.recOffpeakHourly) + '，官网数据）';
              }
              dsChildren.push(React.createElement('div', {
                key: 'an3',
            ...
            if (settings.analyzeMode === 'lowcost' && Number(settings.recDaily) > 0) {
              const recText = '建议: 每日 ≤' + fmtTokens(settings.recDaily) + ' tok · ' + fmtMoney(Number(settings.recCostLimit), settings.currency, rate)
                ...
                + '）· 每项目 ≤' + fmtTokens(settings.recPeakPerProject) + '/时（×' + settings.recParallel + ' 并行）';
              dsChildren.push(React.createElement('div', {
                key: 'an6',
          ...
          dsChildren.push(React.createElement('div', { key: 'save-btns', style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 } },
            ...
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
              title: '分析模式：用量分析（官网当月日均 → 建议限额；未配置官网凭据时回退本地低价区分析；只出建议，不参与设置）',
              onPointerDown: (e) => e.stopPropagation(),
              onClick: (e) => {
                e.stopPropagation();
                const s = loadSettings();
