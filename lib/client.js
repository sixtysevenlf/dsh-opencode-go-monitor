...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, autoMode: 'off', gateOn: false, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 超限断点截断：同步开关与限额到 host（挂载时 + 设置变化时）
          React.useEffect(() => {
          ...
          }, [settings.gateOn, settings.costLimit, settings.tokenLimit]);
          ...
          if (an) {
            ...
            if (settings.gateOn === true) {
              ...
              }, '断点截断: ' + (enf ? (on ? '已超限，将拦截新请求' : (enf.overBudget ? '已超限（拦截待触发）' : '开（限额内）')) : '开') + (enf && enf.overBudget && !on ? '' : '')));
            }
          }
