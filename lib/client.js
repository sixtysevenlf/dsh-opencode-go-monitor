/**
 * 余额悬浮窗（合并版）—— client 半
 * 三标签页：DeepSeek 余额 / 阿里百炼余额 / OpenCode Go 余额，标签切换带滑动+淡入动画。
 * 同源接口：
 *   /api/deepseek/balance     — DeepSeek 余额（30s 轮询）
 *   /api/bailian/balance      — 阿里百炼余额（60s 轮询）
...
`;
    ...
    function loadSettings() {
      try {
        ...
        if (raw) {
          ...
          // 防呆：visible 必须是布尔；历史遗留/损坏值一律回 true，绝不无端隐藏悬浮窗。
    ...
    // 当前是否 DeepSeek 官方高峰时段（北京时间 09-12 / 14-18；周末全天低谷价 → 恒 false）
    const bjDayNow = () => new Date(Date.now() + 8 * 3600000).getUTCDay(); // 0=周日 … 6=周六
    const isWeekendNow = () => { const d = bjDayNow(); return d === 0 || d === 6; };
    ...
    // 时段信息：isPeak（周末恒 false）、isWeekend、距下次时段切换分钟数（周末 → 周一 09:00）
    const periodInfo = () => {
      const bj = new Date(Date.now() + 8 * 3600000);
      const day = bj.getUTCDay(); // 0=周日 … 6=周六
      const h = bj.getUTCHours();
      const minsOfDay = h * 60 + bj.getUTCMinutes();
      if (day === 0 || day === 6) {
        // 周末全天低谷：下次切换 = 周一 09:00（北京时间）
        const elapsed = (day === 6 ? 0 : 24 * 60) + minsOfDay;
        return { isPeak: false, isWeekend: true, mins: Math.max(0, 57 * 60 - elapsed) };
      }
      const isPeak = (h >= 9 && h < 12) || (h >= 14 && h < 18);
      let next;
      if (isPeak) next = (h < 12 ? 12 * 60 : 18 * 60) - minsOfDay;
      else if (h < 9) next = 9 * 60 - minsOfDay;
      else if (h < 14) next = 14 * 60 - minsOfDay;
      else next = 33 * 60 - minsOfDay; // 18:00 后 → 次日 09:00
      return { isPeak, isWeekend: false, mins: Math.max(0, next) };
    };
    // 估算所用单价时段文案（来自 host /api/deepseek/balance）
    const dsPeriodText = (ds) => ds && ds.pricePeriod === 'peak' ? '当前高峰价'
      : (ds && ds.weekend ? '周末低谷价' : '当前空闲价');
    const fmtReset = (iso) => {
      try {
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r4', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', { key: 'hl', style: { flex: 1, color: 'var(--dsw-alias-label-secondary,#888)', cursor: 'help' },
            title: '高价区（高峰 09-12 / 14-18）每个小时的 tok 上限：本值为「基准每小时」，高峰时段自动减半后即高价区上限（例如填基准 1500 万，高峰实际限额 750 万）；周末全天低谷价，不减半' },
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          const [blErr, setBlErr] = React.useState(null);
          ...
          const refreshBl = React.useCallback(async () => {
            try {
              const res = await fetch('/api/bailian/balance');
              const data = await res.json();
              if (data && (data.ok || data.stale)) { setBl(data); setBlErr(null); }
              else { setBl(null); setBlErr(data && data.error ? data.error : '获取失败'); }
            } catch (e) { setBl(null); setBlErr(String((e && e.message) || e)); }
          }, []);
          ...
          const refresh = React.useCallback(() => { refreshGo(); refreshUse(); refreshDs(); refreshBl(); refreshPu(); }, [refreshGo, refreshUse, refreshDs, refreshBl, refreshPu]);
          ...
          React.useEffect(() => {
            ...
            const stop4 = ctx.interval(refreshBl, 60000);
            const stop5 = ctx.interval(refreshPu, 30000);
            return () => { stop1(); stop2(); stop3(); stop4(); stop5(); };
          }, [refresh, refreshGo, refreshUse, refreshDs, refreshBl, refreshPu]);
          ...
          const onPointerMove = (e) => {
            ...
            if (!d) return;
            setPos(clampPos(d.bx + (e.clientX - d.sx), d.by - (e.clientY - d.sy), sizeRef.current.w, sizeRef.current.h));
          };
          const onPointerUp = (e) => {
            const d = dragRef.current;
            ...
            if (d) {
              if (Math.abs(e.clientX - d.sx) + Math.abs(e.clientY - d.sy) < 4) {
                ...
                const next = clampPos(d.bx + (e.clientX - d.sx), d.by - (e.clientY - d.sy), sizeRef.current.w, sizeRef.current.h);
          ...
          const blErrActive = tab === 'bl' && blErr;
          const curErr = tab === 'go' ? (goErr || useErr) : (tab === 'ds' ? dsErr : blErr);
          const curData = tab === 'go' ? go : (tab === 'ds' ? ds : bl);
          ...
          // DeepSeek 官方高低价时段（北京时间高峰 09-12 / 14-18，空闲半价；周末全天低谷价）——纯时间计算，不依赖历史数据
          {
            const pi = periodInfo();
            const isPeakNow = pi.isPeak;
            ...
            dsChildren.push(React.createElement('div', { className: 'opencg-row simple', key: 'w1' },
              ...
              React.createElement('span', { style: { fontSize: 11 } }, '高峰 09-12 / 14-18 (UTC+8) · 空闲半价 · 周末全天低谷价')));
            dsChildren.push(React.createElement('div', { className: 'opencg-row simple', key: 'w2' },
              ...
              React.createElement('span', { style: { fontSize: 11, fontWeight: 600, color: isPeakNow ? '#D97706' : '#16A34A' } },
                isPeakNow ? '高峰 · 距空闲 ' + fmtMin(pi.mins)
                  : (pi.isWeekend ? '周末低谷价 · 距周一高峰 ' + fmtMin(pi.mins)
                    : '空闲(半价) · 距高峰 ' + fmtMin(pi.mins)))));
          }
          // 当前时段建议（纯价格逻辑；周末全天低谷价）
          {
            const pi2 = periodInfo();
            dsChildren.push(React.createElement('div', { key: 'an2', style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary,#888)' } },
              pi2.isPeak ? '建议: 当前高峰时段(2倍价)，重任务等空闲再跑'
                : (pi2.isWeekend ? '建议: 周末全天低谷价，适合跑重任务'
                  : '建议: 当前空闲时段(半价)，适合跑重任务')));
          }
          // 分析建议（官网口径）+ 一键采纳为手动限额
          ...
          dsChildren.push(React.createElement('div', {
            ...
            key: 'st',
            title: dsErr ? '余额: ' + dsErr : (ds ? '预计剩余按 ¥' + ds.pricePerMillion + '/百万 token 估算（' + dsPeriodText(ds) + (ds.cacheHitRate != null ? ' · 缓存命中率' + Math.round(ds.cacheHitRate * 100) + '%' : '') + '）' : ''),
          }, dsStatus));

          // 阿里百炼面板（BSS QueryAccountBalance）
          const blChildren = [];
          const blBalanceText = bl ? fmtBalance(bl.balance, settings.currency, rate) : '--';
          blChildren.push(React.createElement('div', { className: 'opencg-row simple', key: 'b1' },
            React.createElement('span', { className: 'opencg-name' }, '余额'),
            React.createElement('span', { className: 'opencg-value' }, blBalanceText)));
          if (bl && bl.availableCashAmount != null) {
            blChildren.push(React.createElement('div', { className: 'opencg-row simple', key: 'b2' },
              React.createElement('span', { className: 'opencg-name' }, '可用现金'),
              React.createElement('span', { className: 'opencg-value', style: { fontSize: 11 } }, fmtBalance(Number(bl.availableCashAmount) || 0, settings.currency, rate))));
          }
          const blStatus = blErr ? '余额获取失败' : (bl ? '余额 ' + fmtTime(bl.fetchedAt) + ' · 每 60s 刷新' : '余额 --');
          blChildren.push(React.createElement('div', {
            className: blErr ? 'opencg-status err' : 'opencg-status',
            key: 'st',
            title: blErr ? '余额: ' + blErr : (bl ? '阿里云账号余额（QueryAccountBalance）' : ''),
          }, blStatus));

          const paneChildren = tab === 'go' ? goChildren : (tab === 'ds' ? dsChildren : blChildren);
          ...
          const header = React.createElement('div', { className: 'opencg-header', key: 'h' },
            ...
            React.createElement('div', { className: 'opencg-tabs' },
              ...
              React.createElement('button', {
                type: 'button',
                className: tab === 'bl' ? 'opencg-tab active' : 'opencg-tab',
                onClick: () => switchTab('bl'),
                onPointerDown: (e) => e.stopPropagation(),
              }, '百炼'),
              React.createElement('button', {
                type: 'button',
          ...
          if (tab === 'go') {
            ...
            if (bl) {
              tip = '阿里百炼余额 ' + blBalanceText + (bl.currency && bl.currency !== 'CNY' ? ' ' + bl.currency : '')
                + (bl.stale ? ' · 数据过期' : '');
            } else if (blErr) tip = '阿里百炼余额获取失败：' + blErr + '（点击重试）';
          } else {
            if (ds) {
              tip = 'DeepSeek 余额 ' + dsBalanceText + (ds.model ? ' · ' + ds.model : '')
                + ' · 预计剩余 ' + fmtTokens(ds.estTokens) + ' tokens（¥' + ds.pricePerMillion + '/百万 · ' + dsPeriodText(ds) + (ds.cacheHitRate != null ? ' · 命中率' + Math.round(ds.cacheHitRate * 100) + '%' : '') + '）';
