...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, reqLimit: 0, costLimit: 0, peakReqLimit: 0 };
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          if (saveOn && an && an.today) {
            ...
            dsChildren.push(React.createElement('div', {
              key: 'an1',
              style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary,#888)', marginTop: 2 },
            }, '时段分布(北京时间): 高峰 ' + an.peak.msgs + ' 次 $' + an.peak.cost.toFixed(4)
              + ' · 空闲 ' + an.offpeak.msgs + ' 次 $' + an.offpeak.cost.toFixed(4)));
            dsChildren.push(React.createElement('div', {
              key: 'an2',
              style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary,#888)' },
            }, 'DeepSeek 官方空闲时段半价: 高峰请求挪到空闲约省 $' + an.savingsEstimate.toFixed(4) + '/日'));
          }
