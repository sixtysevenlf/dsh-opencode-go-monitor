...
`;
    ...
    // 金额换算（USD → 所选单位）：花费/限额/建议均为 USD 计费
    const fmtMoney = (usd, cur, rate) => {
      if (!Number.isFinite(usd)) return '--';
      const isCny = cur === 'CNY';
      const v = isCny ? usd * rate : usd;
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r5', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          ...
          React.createElement('select', {
            ...
            onChange: (e) => {
              ...
              if (next !== 'off' && settings.analyzeMode === 'off') {
                const ok = window.confirm(
                  '分析模式说明\n\n此模式只「分析」你当前项目近 7 天的 token 用量（重点看低价时段每小时用量），推算出高价时段的建议限额：\n\n· 高价时段 2 倍价 → 建议每小时用量减半\n· 考虑多项目并行：建议按「每项目每小时」给出\n· 不会自动修改任何限额；点「采纳」一键套用到手动限额\n· 关闭后建议值保留，随时可采纳\n\n确定开启？',
                );
                if (!ok) return;
              }
            ...
            React.createElement('option', { value: 'lowcost' }, '低价区分析'),
          ),
        ),
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 分析模式（低价区分析）：近 7 天低价时段每小时 tok 用量 → 高价时段建议限额（只出建议，不参与设置）
          React.useEffect(() => {
            ...
            const lc = h.lowcost;
            if (!lc) return;
            const recOff = lc.offpeakHourly || lc.peakHourly || 0;               // 低价区实际活跃速率（无低价数据时用高峰速率参考）
            const recPeak = Math.round(recOff / 2);                              // 高价区 2 倍价 → 建议每小时减半
            const parallel = Math.max(1, Math.ceil(lc.parallelAvg || 1));        // 并行项目数（平均向上取整）
            const recPer = Math.max(1, Math.round(recPeak / parallel));          // 每项目每小时
            const recDaily = recOff > 0 ? (lc.dailyEstimate || 0) : 0;           // 每日建议：速率 × 活跃日平均使用时长（与速率同一体系）
            const rCost = Math.round((h.avg ? h.avg.cost : 0) * 100) / 100;
            if (settings.recOffpeakHourly !== recOff || settings.recPeakHourly !== recPeak
              || settings.recPeakPerProject !== recPer || settings.recParallel !== parallel
              || settings.recDaily !== recDaily || settings.recCostLimit !== rCost) {
              saveSettings({ ...settings, recOffpeakHourly: recOff, recPeakHourly: recPeak, recPeakPerProject: recPer, recParallel: parallel, recDaily, recCostLimit: rCost });
            }
          }, [use, settings]);
          ...
          if (today) {
            goChildren.push(React.createElement('div', { className: 'opencg-usage', key: 'u1' },
              '今日 ' + fmtTokens(today.input + today.output + today.reasoning) + ' tok · ' + fmtMoney(today.cost, settings.currency, rate) + ' · ' + today.msgs + ' 条'));
          }
          if (total) {
            goChildren.push(React.createElement('div', { className: 'opencg-usage dim', key: 'u2' },
              '累计 ' + fmtTokens(total.input + total.output + total.reasoning) + ' tok · ' + fmtMoney(total.cost, settings.currency, rate) + ' · ' + total.sessions + ' 会话'));
          }
          ...
          if (an) {
            ...
            let advice;
            if (an.peak.msgs > 0) {
              advice = '建议: 高峰 ' + an.peak.msgs + ' 次挪到空闲可省 ~' + fmtMoney(an.savingsEstimate, settings.currency, rate) + '/日（半价）';
            } else if (isPeakNow) {
            ...
            if (use && use.history && use.history.avg) {
              const h = use.history;
              let autoInfo = '';
              if (settings.analyzeMode === 'lowcost') {
                autoInfo = '（分析: 低价区 ≈' + fmtTokens(settings.recOffpeakHourly) + ' tok/时 · 并行 ≈' + settings.recParallel + ' 项目）';
              }
              dsChildren.push(React.createElement('div', {
                key: 'an3',
                style: { fontSize: 10, color: 'var(--dsw-alias-label-secondary,#888)' },
              }, '近' + (h.avg.activeDays || 7) + '天日均: ' + fmtTokens(h.avg.tokens) + ' tok · ' + fmtMoney(h.avg.cost, settings.currency, rate) + '/日' + autoInfo));
            }
            // 分析模式（低价区）：建议参考值 + 一键采纳为手动限额
            if (settings.analyzeMode === 'lowcost' && Number(settings.recDaily) > 0) {
              const recText = '建议: 每日 ≤' + fmtTokens(settings.recDaily) + ' tok · 低价区 ' + fmtTokens(settings.recOffpeakHourly)
                + '/时 · 高价区 ≤' + fmtTokens(settings.recPeakPerProject) + '/时/项目（×' + settings.recParallel + ' 并行）';
              dsChildren.push(React.createElement('div', {
                key: 'an6',
          ...
          dsChildren.push(React.createElement('div', { key: 'save-btns', style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 } },
            ...
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
              title: '分析模式：低价区分析（近 7 天低价时段每小时用量 → 高价区建议限额，只出建议，不参与设置）',
              onPointerDown: (e) => e.stopPropagation(),
              onClick: (e) => {
                e.stopPropagation();
                const s = loadSettings();
            ...
            }, settings.analyzeMode === 'lowcost' ? '分析:低价' : '分析:关'),
          ));
          if (ds && ds.model) {
