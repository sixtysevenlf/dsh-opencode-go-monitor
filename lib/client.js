...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, autoMode: 'off', peakMaxTokensOn: false, peakMaxTokens: 0, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
    ...
    const previewAlerts = () => {
      try {
        ...
        const ov = Number.isFinite(Number(s.overVolume)) ? Math.max(0, Math.min(1, Number(s.overVolume))) : 1;
    ...
    function BalanceSettingsRow() {
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r7', style: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, flexWrap: 'wrap' } },
          ...
          React.createElement('span', { key: 'ovl', style: { color: 'var(--dsw-alias-label-secondary,#888)' } }, '超限音量'),
          React.createElement('input', {
            key: 'ovv',
            type: 'number',
            min: 0,
        ...
        (props) => {
          ...
          React.useEffect(() => {
            ...
            const check = (key, used, cap, nearText, overText) => {
              ...
              const overVol = Number.isFinite(Number(settings.overVolume)) ? Math.max(0, Math.min(1, Number(settings.overVolume))) : 1;
              if (ratio >= 1) {
