      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          React.useEffect(() => {
            ...
            // 每日金额建议：优先官网平台日均金额（当月总花费/活跃日），官网不可用时回退本地官网价口径
            const rCost = Math.round(((pu && pu.ok && pu.dailyAvgCost > 0) ? pu.dailyAvgCost : (h.avg ? h.avg.cost : 0)) * 100) / 100;
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              ...
              }, '近' + (h.avg.activeDays || 7) + '天日均(本地): ' + fmtTokens(h.avg.tokens) + ' tok · ' + fmtMoney(h.avg.cost, settings.currency, rate) + '/日' + autoInfo));
            }
