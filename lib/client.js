...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, analyzeMode: 'off', recCostLimit: 0, recTokenLimit: 0, recHourlyTokenLimit: 0, gateOn: false, gatePeakOnly: false, compressOn: false, compressAtPercent: 80, compressPeakOnly: true, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
    ...
    // 生效限额：仅手动设置参与（分析模式只出建议，不参与）
    const effCaps = (s) => ({
      cost: Number(s.costLimit) || 0,
      tok: Number(s.tokenLimit) || 0,
      hour: Number(s.hourlyTokenLimit) || 0,
    });
    const hasEffectiveCaps = (s) => {
      const e = effCaps(s);
    ...
    function BalanceSettingsRow() {
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r3', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', { key: 'cl', style: { color: 'var(--dsw-alias-label-secondary,#888)' } }, '每日金额上限(手动) $'),
          React.createElement('input', {
            key: 'cost',
            type: 'number',
            min: 0,
        ...
        React.createElement('div', { key: 'r5', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', {
            ...
            title: '分析模式：只分析当前项目 token 用量并给出建议限额参考值，不参与任何具体设置；建议值可在悬浮窗里一键「采纳」为手动限额（也可直接手动填写上方上限）',
          }, '分析模式（建议参考，不参与设置）'),
          React.createElement('select', {
            ...
            onChange: (e) => {
              ...
              if (next !== 'off' && settings.analyzeMode === 'off') {
                const ok = window.confirm(
                  '分析模式说明\n\n此模式只「分析」你当前项目的 token 用量，给出建议限额参考值：\n\n· 不会自动修改任何限额（手动设置保持原样）\n· 建议值会在悬浮窗显示，点「采纳」一键套用到手动限额\n· 关闭后建议值保留，随时可采纳\n\n确定开启？',
                );
                if (!ok) return;
              }
            ...
            React.createElement('option', { value: '7d' }, '7 天平均分析'),
            React.createElement('option', { value: 'hourly' }, '单日小时分析'),
          ),
        ),
        React.createElement('div', { key: 'r6', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          ...
          React.createElement('input', {
            ...
            onChange: (e) => {
              if (e.target.checked) {
                const capsWarn = !hasEffectiveCaps(settings)
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 分析模式：只分析用量并保存建议参考值（rec*），不参与任何限额设置
          React.useEffect(() => {
            const mode = settings.analyzeMode;
            if (mode === 'off') return;
            const h = use && use.history;
            if (!h) return;
            if (mode === '7d') {
              if (!h.avg) return;
              const rTok = h.avg.tokens;
              const rCost = Math.round(h.avg.cost * 100) / 100;
              if (settings.recTokenLimit !== rTok || settings.recCostLimit !== rCost) {
                saveSettings({ ...settings, recTokenLimit: rTok, recCostLimit: rCost });
              }
            } else if (mode === 'hourly') {
              if (!h.refDay) return;
              const rHour = Math.max(1, Math.round(h.refDay.tokens / 24));
              const rTok = h.refDay.tokens;
              const rCost = Math.round(h.refDay.cost * 100) / 100;
              if (settings.recHourlyTokenLimit !== rHour || settings.recTokenLimit !== rTok || settings.recCostLimit !== rCost) {
                saveSettings({ ...settings, recHourlyTokenLimit: rHour, recTokenLimit: rTok, recCostLimit: rCost });
              }
            }
          }, [use, settings]);
          ...
          // 超限断点截断 + 低耗压缩：同步开关与生效限额到 host（挂载时 + 设置变化时）
          React.useEffect(() => {
            const caps = effCaps(settings);
          ...
          React.useEffect(() => {
            ...
            const caps = effCaps(settings);
            const hourlyCapRaw = caps.hour;
            const isPeakNow = a.current && a.current.status === 'peak';
            // 每小时上限在高峰时段自动减半（花费等价）
            const effHour = (hourlyCapRaw > 0 && isPeakNow)
            ...
            check('cost', a.today.cost, caps.cost,
              '今日金额已用百分之九十八，即将达到上限', '今日金额已达上限，建议停止使用');
            check('tok', a.today.tokens, caps.tok,
          ...
          if (an) {
            ...
            if (use && use.history && use.history.avg) {
              const h = use.history;
              let autoInfo = '';
              if (settings.analyzeMode === '7d') {
                autoInfo = '（分析建议: 每日 tok ≈' + fmtTokens(settings.recTokenLimit) + ' · 金额 ≈$' + Number(settings.recCostLimit).toFixed(3) + '/日）';
              } else if (settings.analyzeMode === 'hourly') {
                autoInfo = '（分析建议: 每日 tok ≈' + fmtTokens(settings.recTokenLimit) + ' · 每小时 ≈' + fmtTokens(settings.recHourlyTokenLimit) + '）';
              }
              dsChildren.push(React.createElement('div', {
                key: 'an3',
            ...
            // 分析模式：建议参考值 + 一键采纳为手动限额
            if (settings.analyzeMode !== 'off' && (Number(settings.recCostLimit) > 0 || Number(settings.recTokenLimit) > 0)) {
              const modeLabel = settings.analyzeMode === '7d' ? '7天平均' : '单日小时';
              let recText = '分析建议(' + modeLabel + '): 每日 tok ≈' + fmtTokens(settings.recTokenLimit)
                + ' · 金额 ≈$' + Number(settings.recCostLimit).toFixed(3) + '/日';
              if (settings.analyzeMode === 'hourly' && Number(settings.recHourlyTokenLimit) > 0) {
                recText += ' · 每小时 ≈' + fmtTokens(settings.recHourlyTokenLimit) + '（高峰自动减半）';
              }
              dsChildren.push(React.createElement('div', {
                key: 'an6',
                ...
                React.createElement('button', {
                  ...
                  title: '把分析建议值套用到手动限额（每日金额/tok' + (settings.analyzeMode === 'hourly' ? '/每小时' : '') + '）',
                  ...
                  onClick: (e) => {
                    ...
                    saveSettings({
                      ...s,
                      costLimit: Number(settings.recCostLimit) || 0,
                      tokenLimit: Number(settings.recTokenLimit) || 0,
                      hourlyTokenLimit: settings.analyzeMode === 'hourly' ? (Number(settings.recHourlyTokenLimit) || 0) : (Number(s.hourlyTokenLimit) || 0),
                    });
                  },
                }, '采纳'),
              ));
          ...
          if (saveOn && an && an.today) {
            ...
            const hourlyCapRaw = caps.hour;
            // 每小时上限在高峰时段自动减半（花费等价）
            const effHourlyCap = (hourlyCapRaw > 0 && isPeakNowAny)
              ? Math.max(1, Math.round(hourlyCapRaw / 2))
              : hourlyCapRaw;
          ...
          dsChildren.push(React.createElement('div', { key: 'save-btns', style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 } },
            ...
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
              title: '分析模式：7天平均分析 / 单日小时分析（只出建议，不参与设置）',
              onPointerDown: (e) => e.stopPropagation(),
              onClick: (e) => {
                e.stopPropagation();
                const s = loadSettings();
                const next = s.analyzeMode === 'off' ? '7d' : (s.analyzeMode === '7d' ? 'hourly' : 'off');
                saveSettings({ ...s, analyzeMode: next });
              },
            }, settings.analyzeMode === '7d' ? '分析:7天' : (settings.analyzeMode === 'hourly' ? '分析:小时' : '分析:关')),
            React.createElement('button', {
              type: 'button',
              className: 'opencg-mini',
