...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, reqLimit: 0, costLimit: 0 };
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          if (saveOn && an) {
            ...
            const over = (reqCap > 0 && req > reqCap) || (costCap > 0 && cost > costCap);
            ...
            goChildren.push(budgetRow('请求', req + (reqCap > 0 ? '/' + reqCap : ''), req, reqCap));
            goChildren.push(budgetRow('费用', '$' + cost.toFixed(4) + (costCap > 0 ? '/$' + costCap.toFixed(2) : ''), cost, costCap));
            if (over) {
          ...
          goChildren.push(React.createElement('div', { key: 'save-btns', style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 } },
