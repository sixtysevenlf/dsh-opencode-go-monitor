...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true };
    ...
    // 设置行：显示/隐藏悬浮窗开关（设置 → 常规）
    function BalanceSettingsRow() {
      const [settings, setSettings] = React.useState(loadSettings);
      const onToggle = (e) => {
        const next = { ...settings, visible: e.target.checked };
        setSettings(next);
        saveSettings(next);
      };
      return React.createElement(
        ...
        React.createElement('span', { key: 'label', style: { flex: 1, fontSize: 13 } },
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 设置 → 常规 → 显示/隐藏悬浮窗
          const [visible, setVisible] = React.useState(() => loadSettings().visible !== false);
          React.useEffect(() => subscribeSettings((s) => setVisible(s.visible !== false)), []);
