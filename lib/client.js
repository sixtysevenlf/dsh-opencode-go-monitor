...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, autoMode: 'off', gateOn: false, compressOn: false, compressAtPercent: 80, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
      ...
      return React.createElement(
        ...
        React.createElement('div', { key: 'r8', style: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 } },
          React.createElement('span', {
            ...
            title: '额外功能：用量达到阈值后，大工具结果自动与上次执行结果做行级 diff，只保留变化部分（完全相同则只剩一句"无变化"）——大幅降低上下文与花费',
          }, '低耗压缩（工具结果只留变化）'),
          React.createElement('input', {
            key: 'cpon',
            type: 'checkbox',
            ...
            onChange: (e) => {
              if (e.target.checked) {
                const ok = window.confirm(
                  '⚠ 额外功能确认\n\n开启后，用量达到阈值（默认 80%）时，较长的工具执行结果会被压缩：\n\n· 与上次执行结果做行级对比，只保留变化行\n· 完全相同 → 只剩一句"无变化"\n\n注意：压缩是有损的——模型将看不到被省略的未变化部分，可能影响精细任务。\n\n确定开启？',
                );
                if (!ok) return;
              }
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          // 超限断点截断 + 低耗压缩：同步开关与限额到 host（挂载时 + 设置变化时）
          React.useEffect(() => {
            fetch('/api/opencode-go/config', {
              ...
              body: JSON.stringify({
                ...
                costLimit: Number(settings.costLimit) || 0,
                tokenLimit: Number(settings.tokenLimit) || 0,
                compressEnabled: settings.compressOn === true,
                compressAtPercent: Number(settings.compressAtPercent) || 80,
              }),
            }).catch(() => { /* 下次重试 */ });
          }, [settings.gateOn, settings.costLimit, settings.tokenLimit, settings.compressOn, settings.compressAtPercent]);
          ...
          if (an) {
            ...
            if (settings.gateOn === true) {
              ...
              dsChildren.push(React.createElement('div', {
                key: 'an4',
                style: { fontSize: 10, color: on ? '#DC2626' : 'var(--dsw-alias-label-secondary,#888)' },
              }, '断点截断: ' + (enf ? (on ? '已超限，将拦截新请求' : (enf.overBudget ? '已超限（拦截待触发）' : '开（限额内）')) : '开')));
            }
            if (settings.compressOn === true) {
              ...
              dsChildren.push(React.createElement('div', {
                key: 'an5',
                style: { fontSize: 10, color: act ? '#D97706' : 'var(--dsw-alias-label-secondary,#888)' },
              }, '低耗压缩: ' + (enf ? (act ? '已激活（阈值 ' + enf.compressAtPercent + '%）· 压缩 ' + enf.compressCount + ' 次' : '开（未达阈值）') : '开')));
            }
          }
