...
`;
    ...
    const DEFAULT_SETTINGS = { visible: true, saveMode: false, costLimit: 0, tokenLimit: 0, hourlyTokenLimit: 0, autoMode: 'off', gateOn: false, compressOn: false, compressAtPercent: 80, compressPeakOnly: true, soundAlert: true, nearVolume: 0.3, overVolume: 1 };
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        (props) => {
          ...
          }, [settings.gateOn, settings.costLimit, settings.tokenLimit, settings.compressOn, settings.compressAtPercent, settings.compressPeakOnly]);
          ...
          if (an) {
            ...
            if (settings.compressOn === true) {
              ...
              }, '低耗压缩: ' + (enf ? (act ? '已激活（阈值 ' + enf.compressAtPercent + '%' + (peakOnly ? '· 高峰' : '') + '）· 压缩 ' + enf.compressCount + ' 次' : '开（未达激活条件）' + (peakOnly ? '· 仅高峰' : '')) : '开')));
            }
          }
