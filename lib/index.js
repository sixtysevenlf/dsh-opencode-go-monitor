...
async function queryUsage() {
  ...
  try {
    ...
    let peakMsgs = 0, peakCost = 0, offMsgs = 0, offCost = 0
    ...
    for (const r of rows) {
      ...
      if (isToday) {
        ...
        if (bh >= 9 && bh < 12 || bh >= 14 && bh < 18) { peakMsgs += 1; peakCost += cost }
      ...
      if (dk >= todayKey - 6) {
        let e = hist.get(dk)
        if (!e) { e = { msgs: 0, cost: 0, peakMsgs: 0 }; hist.set(dk, e) }
        e.msgs += 1; e.cost += cost
        const bh2 = bjHour(created)
        if ((bh2 >= 9 && bh2 < 12) || (bh2 >= 14 && bh2 < 18)) e.peakMsgs += 1
      }
    }
    ...
    const days = []
    for (let i = 6; i >= 0; i--) {
      const e = hist.get(todayKey - i) || { msgs: 0, cost: 0, peakMsgs: 0 }
      days.push({ msgs: e.msgs, cost: Math.round(e.cost * 1000) / 1000, peakMsgs: e.peakMsgs })
    }
    const avg = {
    ...
    return {
      ...
      last,
      analysis: {
        today: { msgs: today.msgs, cost: today.cost },
        peak: { msgs: peakMsgs, cost: peakCost },
        offpeak: { msgs: offMsgs, cost: offCost },
