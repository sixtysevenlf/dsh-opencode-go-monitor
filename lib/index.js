 ...
 *   GET /api/bailian/balance     — 阿里云百炼/账号余额（BSS QueryAccountBalance，需 AccessKey）
 *
 ...
 *   阿里百炼   ：DSH 凭据 ALIBABA_ACCESS_KEY_ID / ALIBABA_ACCESS_KEY_SECRET
 */
...
// ---------- 峰谷计费（2026-08-17 起实施；2026-08-23 起周末全天低谷价） ----------
// 官方价格（¥/百万 tokens，低谷价；高峰 = 低谷 × 2）：
//   deepseek-v4-flash / deepseek-v4-flash-vision-exp：输入(缓存命中) 0.05 / 输入(未命中) 1.5 / 输出 4.5
//   deepseek-v4-pro：输入(缓存命中) 0.15 / 输入(未命中) 4.5 / 输出 13.5
// 高峰时段（北京时间）：09:00-12:00、14:00-18:00；其余为空闲(低谷)时段
// 周末（周六/周日）：全天统一按低谷价，不再区分峰谷时段
const DS_MODEL_PRICES = {
  'deepseek-v4-flash': { input: 1.5, inputHit: 0.05, output: 4.5 },
  'deepseek-v4-flash-vision-exp': { input: 1.5, inputHit: 0.05, output: 4.5 },
  'deepseek-v4-pro': { input: 4.5, inputHit: 0.15, output: 13.5 },
  // 旧模型名兼容（按官方等价模型映射）
  'deepseek-chat': { input: 1.5, inputHit: 0.05, output: 4.5 },
  'deepseek-reasoner': { input: 4.5, inputHit: 0.15, output: 13.5 },
}
const DS_DEFAULT_PRICE = DS_MODEL_PRICES['deepseek-v4-flash']
// 平均缓存命中率参考（官网平台当月输入侧命中率；官网数据未到手时的默认假设）
const DS_DEFAULT_CACHE_HIT_RATE = 0.5
let dsOverallHitRate = null // 平台全模型输入侧命中率（null=尚无数据）
let dsModelHitRates = new Map() // model → 输入侧命中率

// 实际综合单价覆盖（¥/百万 tokens）：
// 当官网平台用量数据不可用时，作为兜底单价估算“预计剩余 token”。
// 若官网平台可用，则自动按 当月日均花费 ÷ 当月日均 token 计算实际单价。
// 当前兜底值按用户历史日均：22.62 元 / 1.84 亿 tok ≈ 0.123 元/百万 tok。
const DS_ACTUAL_BLENDED_PRICE = 0.12

// 北京时间：星期几（0=周日…6=周六）与小时
function bjNow() {
  const bj = new Date(Date.now() + 8 * 3600000)
  return { day: bj.getUTCDay(), hour: bj.getUTCHours() }
}
// 周末（周六/周日）全天低谷价
...
// 当前是否高峰时段（北京时间 09-12 / 14-18；周末恒 false）
function inPeakNow() {
  if (isWeekendNow()) return false
  const h = bjNow().hour
  return (h >= 9 && h < 12) || (h >= 14 && h < 18)
}
// 当前模型在当前时段的单价（¥/百万 tokens）：
//   输入 = 缓存命中率加权：命中价×命中率 + 未命中价×(1-命中率)；输出按官方价
//   命中率优先级：本次实算 → 平台按模型 → 平台总体 → 默认假设
function dsPriceNow(modelKey, hitRateOverride = null) {
  const base = DS_MODEL_PRICES[modelKey] || DS_DEFAULT_PRICE
  const peak = inPeakNow()
  const factor = peak ? 2 : 1
  let r = hitRateOverride
  if (r === null || !Number.isFinite(r)) {
    const perModel = modelKey ? dsModelHitRates.get(modelKey) : null
    r = (perModel !== null && perModel !== undefined && Number.isFinite(perModel)) ? perModel : dsOverallHitRate
  }
  if (r === null || !Number.isFinite(r)) r = DS_DEFAULT_CACHE_HIT_RATE
  r = Math.max(0, Math.min(1, r))
  const input = (base.input * (1 - r) + base.inputHit * r) * factor
  const output = base.output * factor
  return {
    modelKey: DS_MODEL_PRICES[modelKey] ? modelKey : 'deepseek-v4-flash',
    peak,
    weekend: isWeekendNow(),
    cacheHitRate: Math.round(r * 10000) / 10000,
    input,
    output,
    // 估算用混合价（加权输入 + 输出 的平均）
...
async function queryDeepseekBalance(ctx) {
  ...
  // 参考实际平均缓存命中率（官网平台当月输入侧命中率；只用已抓取的缓存，不阻塞余额响应）
  let hitRate = null
  const puCached = platformUsageCache
  if (puCached && puCached.ok && puCached.category) {
    const totalIn = (puCached.category.cacheHit || 0) + (puCached.category.cacheMiss || 0)
    if (totalIn > 0) hitRate = (puCached.category.cacheHit || 0) / totalIn
  }
  const price = dsPriceNow(model, hitRate)
  // 自动按用户平均用量估算实际单价：日均花费 ÷ 日均 token（官网平台数据可用时）
  // 不可用时回退到 DS_ACTUAL_BLENDED_PRICE（当前 0.12）
  let actualBlended = null
  if (puCached && puCached.ok && puCached.dailyAvgTokens > 0 && puCached.dailyAvgCost > 0) {
    actualBlended = (puCached.dailyAvgCost / puCached.dailyAvgTokens) * 1e6
  }
  price.blended = actualBlended && actualBlended > 0 ? actualBlended : DS_ACTUAL_BLENDED_PRICE
  const estTokens = Math.floor(balance / (price.blended / 1e6))
  return {
    ok: true,
    balance,
    ...
    estTokens,
    model,
    pricePerMillion: Math.round(price.blended * 1000) / 1000,
    priceModel: price.modelKey,
    pricePeriod: price.peak ? 'peak' : 'valley',
    weekend: price.weekend,
    cacheHitRate: price.cacheHitRate,
...
// ---------- 阿里云百炼余额（BSS QueryAccountBalance） ----------
const BAILIAN_BALANCE_URL = 'https://business.aliyuncs.com/'
const BAILIAN_CACHE_MS = 60000
...
function aliyunSign(params, secret) {
  ...
  const canonical = keys.map((k) => rfc3986Encode(k) + '=' + rfc3986Encode(String(params[k]))).join('&')
  const stringToSign = 'GET&%2F&' + rfc3986Encode(canonical)
  return createHmac('sha1', secret + '&').update(stringToSign, 'utf8').digest('base64')
}

async function resolveAlibabaKey(ctx) {
  try {
    const idCred = await ctx.credentials.resolve('ALIBABA_ACCESS_KEY_ID')
    const secCred = await ctx.credentials.resolve('ALIBABA_ACCESS_KEY_SECRET')
    if (idCred && idCred.value && secCred && secCred.value) {
      return { id: idCred.value, secret: secCred.value }
    }
  } catch { /* 走环境变量 */ }
  if (process.env.ALIBABA_ACCESS_KEY_ID && process.env.ALIBABA_ACCESS_KEY_SECRET) {
    return { id: process.env.ALIBABA_ACCESS_KEY_ID, secret: process.env.ALIBABA_ACCESS_KEY_SECRET }
  }
  throw new Error('未配置 ALIBABA_ACCESS_KEY_ID / ALIBABA_ACCESS_KEY_SECRET 凭据')
}

async function queryBailianBalance(ctx) {
  const { id, secret } = await resolveAlibabaKey(ctx)
  const params = {
    AccessKeyId: id,
  ...
  const signature = aliyunSign(params, secret)
  params.Signature = signature
  const qs = Object.keys(params).sort().map((k) => rfc3986Encode(k) + '=' + rfc3986Encode(String(params[k]))).join('&')
  const resp = await fetch(BAILIAN_BALANCE_URL + '?' + qs, { signal: AbortSignal.timeout(15000) })
  if (!resp.ok) {
    const text = await resp.text()
    throw new Error(`阿里百炼余额 HTTP ${resp.status}: ${String(text).slice(0, 120)}`)
  }
  const data = await resp.json()
  if (data.Code && data.Code !== '200' && data.Code !== 'Success') {
    throw new Error(`阿里百炼余额 API ${String(data.Code).slice(0, 30)}: ${String(data.Message || '').slice(0, 120)}`)
  }
  const d = data.Data || {}
  const balance = Number.parseFloat(d.AvailableCashAmount ?? d.AvailableAmount ?? d.AvailableBalance)
  if (!Number.isFinite(balance)) throw new Error('阿里百炼余额响应格式异常')
  return {
    ok: true,
    balance,
    currency: d.Currency || 'CNY',
    availableCashAmount: d.AvailableCashAmount,
    availableAmount: d.AvailableAmount,
    creditLimit: d.CreditLimit,
    myBankCredit: d.MyBankCredit,
    fetchedAt: Date.now(),
  }
}
...
async function queryDeepseekPlatformUsage(ctx) {
  ...
  const modelRates = new Map() // 每模型输入侧命中率（无输入数据 → null）
  for (const m of ab.total || []) {
    let h = 0, ms = 0
    for (const it of (m.usage || [])) {
      const amt = Number(it.amount) || 0
      if (it.type === 'PROMPT_CACHE_HIT_TOKEN') { cat.cacheHit += amt; h += amt }
      else if (it.type === 'PROMPT_CACHE_MISS_TOKEN') { cat.cacheMiss += amt; ms += amt }
      else if (it.type === 'RESPONSE_TOKEN') cat.response += amt
    }
    modelRates.set(m.model, h + ms > 0 ? h / (h + ms) : null)
  }
  // 同步平均缓存命中率（余额估算参考；输入侧命中率 = 命中输入 / (命中+未命中) 输入）
  const totalInput = cat.cacheHit + cat.cacheMiss
  dsOverallHitRate = totalInput > 0 ? cat.cacheHit / totalInput : null
  dsModelHitRates = modelRates
  return {
    ok: true,
    month, year,
...
export function apply(ctx) {
  ...
  async function statusBailian() {
    const now = Date.now()
    if (blCache && now - blAt < BAILIAN_CACHE_MS) return blCache
    try {
      const payload = await queryBailianBalance(ctx)
      blCache = payload
      blAt = now
      return payload
    } catch (e1) {
      try {
        ...
        const msg = String((e2 && e2.message) || e2).slice(0, 200)
        if (blCache) return { ...blCache, stale: true, error: msg }
        return { ok: false, error: msg }
      }
    }
  }
  ...
  ctx.effect(() => {
    ...
    disposeRoutes.push(ctx.webServer.register({
      kind: 'exact',
      path: '/api/bailian/balance',
      handler: async (_req, res) => {
        try { json(res, await statusBailian()) } catch (e) { json(res, { ok: false, error: String((e && e.message) || e).slice(0, 200) }, 500) }
      },
    }))
