 ...
 *   GET /api/opencode-go/usage   — 本地 opencode.db 用量（消息级聚合，无外部请求）
 * key 从 ~/.local/share/opencode/auth.json 读取，仅在进程内存中使用，不落盘不外发。
 */
...
const AUTH_PATH = process.env.OGM_AUTH || join(DATA_DIR, 'auth.json')
...
// ---------- 余额（官方接口） ----------
function readKey() {
  if (!existsSync(AUTH_PATH)) throw new Error(`auth.json 不存在: ${AUTH_PATH}`)
  const auth = JSON.parse(readFileSync(AUTH_PATH, 'utf8'))
  const key = auth[PROVIDER] && auth[PROVIDER].key
  if (!key) throw new Error(`auth.json 中没有 ${PROVIDER} 的 key`)
  return key
}
...
async function queryUsage() {
  ...
  if (!sqlite) return { ok: false, error: 'node:sqlite 不可用（需要 Node.js >= 22.5）' }
  if (!existsSync(DB_PATH)) return { ok: false, error: `数据库不存在: ${DB_PATH}` }
...
export function apply(ctx) {
  ...
  async function statusBalance() {
    ...
    try {
      const payload = await queryBalance()
