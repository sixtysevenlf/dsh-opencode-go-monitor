...
export function apply(ctx) {
  ...
  // 拦截 agent 请求：超出限额时停在工具断点，不再发起新请求（思考链完整）
    ...
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          json(res, { ok: true, gateEnabled, costLimit: gateCostLimit, tokenLimit: gateTokenLimit, overBudget: over, active: gateEnabled && over })
        } catch (e) { json(res, { ok: false, error: String((e && e.message) || e).slice(0, 200) }, 500) }
      },
    })
    ctx.webServer.register({
