...
// ---------- 高峰 max_tokens 强制（真·控制：写 llm-deepseek 模型目录，适配器每次调用带上 max_tokens） ----------
let peakCap = 0          // 0 = 关闭（POST /api/opencode-go/config 设置）
let origModels = null    // 首次应用前保存的 models（null = 继承默认目录）
let enforced = false

function bjHourOf(ms) { return Math.floor(((ms + 8 * 3600000) % 86400000) / 3600000) }
function inPeakNow() {
  const h = bjHourOf(Date.now())
  return (h >= 9 && h < 12) || (h >= 14 && h < 18)
}

async function currentModelId(ctx) {
  try {
    const s = ctx.get('settings').get('agent-default-model')
    return (s && s.model) || null
  } catch { return null }
}

async function applyPeakCap(ctx) {
  const settingsSvc = ctx.get('settings')
  if (!settingsSvc) return false
  const cur = settingsSvc.get('llm-deepseek') || {}
  origModels = Array.isArray(cur.models) ? JSON.parse(JSON.stringify(cur.models)) : null
  const modelId = await currentModelId(ctx)
  if (!modelId) return false
  let next
  if (origModels) {
    next = origModels.map((m) => (m.id === modelId ? { ...m, maxTokens: peakCap } : m))
    if (!next.some((m) => m.id === modelId)) next = [...next, { id: modelId, maxTokens: peakCap }]
  } else {
    next = [{ id: modelId, maxTokens: peakCap }]
  }
  await settingsSvc.mutate('llm-deepseek', [{ op: 'set', path: ['models'], value: next }])
  enforced = true
  return true
}

async function restorePeakCap(ctx) {
  const settingsSvc = ctx.get('settings')
  if (!settingsSvc || !enforced) return true
  if (origModels === null) {
    await settingsSvc.mutate('llm-deepseek', [{ op: 'unset', path: ['models'] }])
  } else {
    await settingsSvc.mutate('llm-deepseek', [{ op: 'set', path: ['models'], value: origModels }])
  }
  enforced = false
  return true
}

async function evaluatePeak(ctx) {
  if (peakCap <= 0) { if (enforced) await restorePeakCap(ctx); return }
  if (inPeakNow()) { if (!enforced) await applyPeakCap(ctx) } else { if (enforced) await restorePeakCap(ctx) }
}
...
export function apply(ctx) {
    ...
    ctx.webServer.register({
      ...
      handler: async (_req, res) => {
        try {
          ...
          json(res, { ...payload, enforcement: { peakMaxTokens: peakCap, active: enforced } })
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'POST') {
            ...
            if (Number.isFinite(body.peakMaxTokens)) {
              peakCap = Math.max(0, Math.floor(body.peakMaxTokens))
            }
            if (body.force === true && peakCap > 0) await applyPeakCap(ctx)
            else if (body.force === 'restore') await restorePeakCap(ctx)
            else await evaluatePeak(ctx)
          }
          json(res, { ok: true, peakMaxTokens: peakCap, active: enforced, inPeak: inPeakNow() })
        } catch (e) { json(res, { ok: false, error: String((e && e.message) || e).slice(0, 200) }, 500) }
      },
    })
    ctx.webServer.register({
  ...
  // 高峰 max_tokens 定时评估（每分钟；ctx.effect 生命周期自动清理）
  const peakTimer = setInterval(() => { evaluatePeak(ctx).catch(() => { /* 评估失败静默，下轮重试 */ }) }, 60000)
  ctx.effect(() => () => clearInterval(peakTimer), 'peak max_tokens evaluator')
}
