...
async function queryUsage() {
  ...
  try {
    ...
    const hourProj = new Map() // 小时键 → 该小时活跃项目集合（并行分析）
    ...
    for (const r of rows) {
      ...
      if (dk >= todayKey - 6) {
        ...
        const hk = dk * 24 + bh2
        let ps = hourProj.get(hk)
        if (!ps) { ps = new Set(); hourProj.set(hk, ps) }
        ps.add(projName.get(r.session_id) || '?')
      }
    }
    ...
    // 低价区分析：低价时段每小时 tok 用量 + 并行项目数（近 7 天）
    const hourSizes = [...hourProj.values()]
    const parallelAvg = hourSizes.length ? Math.round((hourSizes.reduce((s2, set2) => s2 + set2.size, 0) / hourSizes.length) * 10) / 10 : 0
    const parallelMax = hourSizes.length ? Math.max(...hourSizes.map((set2) => set2.size)) : 0
    const lowcost = {
      offpeakHourly: Math.round((sum7Tokens - sum7PeakTokens) / 7 / 18), // 低价时段每小时 tok 用量
      parallelAvg,
      parallelMax,
    }
