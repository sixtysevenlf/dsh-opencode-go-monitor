...
// ---------- DeepSeek 官网价（2026-08-17 生效，元/百万 tokens） ----------
// 峰谷定价：高峰（北京时间 09-12 / 14-18）= 空闲 ×2；空闲时段价格为高峰的一半。
// 用量金额一律按官网价 × 本地 token 用量计算（CNY），不再采用网关记账 cost。
const DS_PRICE_TABLE = {
  'deepseek-v4-flash': { in: 1.5, cache: 0.05, out: 4.5 }, // 空闲：未命中输入/缓存命中输入/输出
  'deepseek-v4-pro': { in: 4.5, cache: 0.15, out: 13.5 },
}
const DS_PRICE_FALLBACK = DS_PRICE_TABLE['deepseek-v4-flash']
function dsPriceFor(mid) {
  const m = String(mid || '').toLowerCase()
  for (const k of Object.keys(DS_PRICE_TABLE)) if (m.startsWith(k)) return DS_PRICE_TABLE[k]
  return DS_PRICE_FALLBACK
}
// 一条消息的官网价消耗（CNY）：缓存命中输入按缓存价，reasoning 按输出价；高峰 ×2
function deepseekCost(mid, tin, tout, trea, tcr, bh) {
  const p = dsPriceFor(mid)
  const mult = ((bh >= 9 && bh < 12) || (bh >= 14 && bh < 18)) ? 2 : 1
  return ((tin * p.in + tcr * p.cache + (tout + trea) * p.out) / 1e6) * mult
}
...
async function queryDeepseekPlatformUsage(ctx) {
  ...
  let totalCost = 0
  for (const m of cb.total || []) totalCost += Number(m.amount) || 0
  const currency = cb.currency || 'CNY'

  // 每日明细合并（amount days × cost days）
  const dayMap = new Map()
  for (const d of ab.days || []) {
    const { tokens, requests } = sumUsage(d.data)
  ...
  for (const d of cb.days || []) {
    let cost = 0
    for (const it of d.data || []) cost += Number(it.amount) || 0
    let e = dayMap.get(d.date)
    if (!e) { e = { date: d.date, tokens: 0, cost: 0, requests: 0 }; dayMap.set(d.date, e) }
    e.cost += cost
  }
...
async function queryUsage() {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const bh0 = bjHour(created)
      const cost = deepseekCost(mid, tin, tout, trea, tcr, bh0) // DeepSeek 官网价消耗（CNY，含高峰×2/空闲半价）
    ...
    // 日均按「实际有使用的天数」计算（跑几天除以几，最多 7 天），不摊到空闲日
    const activeDays7 = days.reduce((s, d) => s + (d.msgs > 0 ? 1 : 0), 0)
    const d7 = Math.max(1, activeDays7)
    const avg = {
      msgs: Math.round(days.reduce((s, d) => s + d.msgs, 0) / d7),
      cost: Math.round(days.reduce((s, d) => s + d.cost, 0) / d7 * 1000) / 1000,
      tokens: Math.round(days.reduce((s, d) => s + d.tokens, 0) / d7),
      peakMsgs: Math.round(days.reduce((s, d) => s + d.peakMsgs, 0) / d7),
    ...
    const rateByWindow = (recs) => {
      ...
      const activeDays = new Set(wins.map((w) => dayKey(w.start))).size
      return {
    ...
    // 每日建议 = 实际速率 × 活跃日平均使用时长（分钟→小时），与速率同一体系
    const dailyEst = lowOff && lowOff.hourly > 0
      ? Math.round((lowOff.hourly * lowOff.winTotalMin) / Math.max(1, lowOff.activeDays) / 60)
      : (lowPeak && lowPeak.hourly > 0 ? Math.round((lowPeak.hourly * lowPeak.winTotalMin) / Math.max(1, lowPeak.activeDays) / 60) : 0)
    const lowcost = {
      ...
      activeDays: lowOff ? lowOff.activeDays : 0,
    ...
    return {
      ...
      provider: PROVIDER,
      db: DB_PATH,
      costCurrency: 'CNY', // 全部 cost 字段按 DeepSeek 官网价（2026-08-17 峰谷定价）计算，人民币
      today,
      total,
