...
- **限额按钮**（DeepSeek 页）：
  - 「省钱」开关：显示每日**请求数/费用**进度条与**高峰时段请求数**进度条，超限红色告警
  - 「自动」开关：**自动限额** —— 按近 7 天日均用量自动设置每日/高峰/费用上限，每次用量刷新自动校准
  - 「一键限额」：一键把每日上限与高峰上限设为今日已用量（超限即提醒）
  - 设置 → 常规：`每日请求上限` / `每日费用上限` / `高峰期请求上限（09-12 / 14-18 内）` 手动输入，另有「自动限额」开关
